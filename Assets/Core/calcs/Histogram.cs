...
//             NativeArray<int> counts = new NativeArray<int>(histogramNumber * Histogram.MAX_BIN_NUMBER, Allocator.TempJob);
//             NativeArray<TComponent> components = AtomQuery.ToComponentDataArray<TComponent>(Allocator.TempJob);
...
//             var getCount = Entities.WithAll<Histogram>().ForEach((Entity entity, int index, in Histogram histogram) => {
...
//                     counts[binId + offset]++;
//                 }
//             }).WithoutBurst().Run(HistogramQuery,inputDeps);
//             // return getCount;
...
//             // {
//             var assignCounts = Entities.WithAll<DataPoint, Histogram>().ForEach((Entity entity, DynamicBuffer<DataPoint> buffer, int index, in Histogram histogram) => {
//                 int offset = histogram.GetOffset(index);
//                 buffer.Clear();
//                 for (int i = 0; i < histogram.BinNumber; i++)
//                     buffer.Add(new DataPoint { Value = counts[offset + i] });
//             }).WithoutBurst().Run(HistogramQuery,getCount);
